...
from address import ADDRESS_KEY_LIST
...
class FormatStr:
    ...
    def get_formatted_headers() -> str:
        ...
        s += "\n{:<10} | {:<20} | {:<12} | {:<11} | {:<35} | {:<26} |\n".format(
            "Number", "User name", "Phone number", "Birthday", "Email", "Address"
        )
        return s
    ...
    def get_formatted_headers_birthdays() -> str:
        ...
        s += "\n{:<10} | {:<20} | {:<70} |\n".format(
            "Name", "Info birthdays", "Days for birthday"
        )
        return s
    ...
    def show_address_book(records: Generator) -> str:
        ...
        for record_list in records:
            for record in record_list:
                phones_list: list = (
                    record[1]["phones"] if record[1]["phones"] else ["None"]
                )
                emails_list: list = (
                    record[1]["emails"] if record[1]["emails"] else ["None"]
                )
